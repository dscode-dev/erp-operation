...
import { ArrowLeft, CalendarClock, CheckCircle2, Download, Eye, FileSignature, FileText, History, Images, PauseCircle, Pencil, Play, RefreshCw, RotateCcw, Settings2, UserPlus, XCircle } from "lucide-react";
...
import {
  ApiClientError, documentsApi, operationApi, pmocApi, usersApi, useQuery,
  type CreateOperationPayload, type DocumentHandoff, type PmocExecutionRequest, type PmocHistoryItem, type PmocPlan,
} from "@erp/api";
...
export default function PmocDetailPage() {
  ...
  const [assigning, setAssigning] = useState<PmocExecutionRequest | null>(null);
  const [concluding, setConcluding] = useState<PmocExecutionRequest | null>(null);
  ...
  // Conclusão pelo owner na plataforma: aprova a coleta em revisão, conclui a OS
  // existente ou gera uma OS já concluída (quando ainda não há OS gerada).
  async function concludeOnPlatform(request: PmocExecutionRequest) {
    setError(null);
    try {
      const operation = request.operation;
      if (operation) {
        if (operation.status === "REVIEW") await operationApi.approveOperation(operation.id);
        else await operationApi.updateOperation(operation.id, { status: "COMPLETED", completedAt: new Date().toISOString() });
      } else {
        const base = await pmocApi.getExecutionRequestPrefill(request.id);
        await pmocApi.generateWorkOrder(request.id, { ...base, documentType: "PMOC", status: "COMPLETED", completedAt: new Date().toISOString() });
      }
      setTick((value) => value + 1);
    } catch (cause) {
  ...
  return <div className="space-y-6">
    ...
    <AssignOperatorDrawer request={assigning} onClose={() => setAssigning(null)} onSaved={() => { setAssigning(null); setTick((value) => value + 1); }} onError={setError} />
    <ConfirmDialog
      open={Boolean(concluding)}
      title={concluding?.operation?.status === "REVIEW" ? "Aprovar e concluir?" : "Concluir na plataforma?"}
      confirmLabel={concluding?.operation?.status === "REVIEW" ? "Aprovar e concluir" : "Concluir"}
      description={
        concluding?.operation?.status === "REVIEW"
          ? "Os dados coletados pelo operador serão aprovados e a execução será concluída."
          : concluding?.operation
            ? "A operação será marcada como concluída diretamente na plataforma."
            : "Uma OS será criada e concluída diretamente por você, sem coleta em campo."
      }
    ...
    <PmocPlanWizard open={editing} pmoc={plan} editMode onClose={() => setEditing(false)} onCreated={() => undefined} onUpdated={() => setTick((value) => value + 1)} />
...
function AssignOperatorDrawer({ request, onClose, onSaved, onError }: { request: PmocExecutionRequest | null; onClose: () => void; onSaved: () => void; onError: (message: string) => void }) {
  const users = useQuery((signal) => usersApi.listUsers({ limit: 100, signal }), []);
  const [operatorId, setOperatorId] = useState("");
  const [busy, setBusy] = useState(false);
  useEffect(() => { setOperatorId(request?.plannedOperator?.id ?? ""); }, [request]);
  const available = users.data?.items.filter((item) => item.isActive && item.role !== "VIEWER") ?? [];
  return <Drawer open={Boolean(request)} onClose={onClose} eyebrow="Execução PMOC" title={`Atribuir execução ${String(request?.executionNumber ?? 0).padStart(3, "0")} a um operador`} footer={<><button className={secondary} onClick={onClose}>Cancelar</button><button className={primary} disabled={!operatorId || busy} onClick={async () => { if (!request) return; setBusy(true); try { const base = await pmocApi.getExecutionRequestPrefill(request.id); await pmocApi.generateWorkOrder(request.id, { ...base, documentType: "PMOC", operatorId, status: "PENDING" }); onSaved(); } catch (cause) { onError(message(cause)); onClose(); } finally { setBusy(false); } }}>Atribuir ao operador</button></>}>
    <div className="space-y-4">
      <label className="grid gap-2 text-sm font-medium">Operador responsável<select value={operatorId} onChange={(event) => setOperatorId(event.target.value)}><option value="">Selecione um operador</option>{available.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.role}</option>)}</select></label>
      <p className="text-sm text-[var(--color-muted-foreground)]">A OS desta execução será criada e enviada ao operador para coleta em campo. Quando o operador enviar os dados coletados, a operação ficará em <strong>revisão</strong> até você aprovar e concluir na plataforma.</p>
    </div>
  </Drawer>;
}
