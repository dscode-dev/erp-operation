...
export default function PmocDetailPage() {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-7"><Metric label="Ciclos previstos" value={overview?.expectedExecutions ?? plan.plannedExecutionCount} /><Metric label="Execuções dos ativos" value={overview?.expectedEquipmentExecutions ?? "—"} /><Metric label="Concluídas" value={overview?.completedExecutions ?? "—"} /><Metric label="Pendentes" value={overview?.remainingExecutions ?? "—"} /><Metric label="Falhas" value={overview?.failedExecutions ?? "—"} tone="danger" /><Metric label="Canceladas" value={overview?.cancelledExecutions ?? "—"} /><Metric label="Próxima" value={date(plan.nextExecutionDate)} /></div>
    {overview?.hasOpenExecutions && <section className="rounded-xl border border-amber-500/40 bg-amber-500/10 p-4 text-sm text-amber-800"><strong>A cobertura deste PMOC terminou, mas ainda existem execuções pendentes.</strong><p className="mt-1">O plano permanecerá aberto até que todos os equipamentos completem seus ciclos previstos. Consulte os itens destacados na aba Execuções.</p></section>}
    {overview && <section className="rounded-xl border border-[var(--color-border)] bg-[var(--color-card)] p-4"><div className="flex items-center justify-between text-sm"><strong>Progresso das execuções por equipamento</strong><span>{overview.completedExecutions} / {overview.expectedEquipmentExecutions} · {overview.remainingExecutions} restantes</span></div><div className="mt-2 h-3 overflow-hidden rounded-full bg-[var(--color-muted)]"><div className="h-full rounded-full bg-[var(--color-primary)]" style={{ width: `${overview.completionPercentage}%` }} /></div><p className="mt-2 text-xs text-[var(--color-muted-foreground)]">Saúde {overview.health.label} ({overview.health.score}/100) · atraso médio {overview.averageDelayDays} dia(s)</p></section>}
    ...
    <Drawer open={Boolean(documentRequest)} onClose={() => setDocumentRequestId(null)} eyebrow="Documento PMOC" title={documentRequest ? `Execução ${String(documentRequest.equipmentExecutionNumber).padStart(3, "0")}` : "Documento"} width="max-w-[1280px]">
      {documentRequest?.operation && <div className="space-y-4">
        ...
        <DocumentViewer
          ...
          title={`PMOC · Execução ${String(documentRequest.equipmentExecutionNumber).padStart(3, "0")}`}
          canRender={canEdit}
...
function PmocDocumentActions({ request, canRender, onOpen }: { request: PmocExecutionRequest; canRender: boolean; onOpen: () => void }) {
  ...
  return <section className="flex flex-col gap-4 rounded-xl border border-[var(--color-border)] bg-[var(--color-card)] p-4 lg:flex-row lg:items-center lg:justify-between">
    <div>
      ...
      <p className="mt-1 text-sm text-[var(--color-muted-foreground)]">Execução {String(request.equipmentExecutionNumber).padStart(3, "0")}{document ? ` · ${document.number} · versão ${document.revision}` : " · o PDF oficial ainda não foi gerado"}</p>
    </div>
    <div className="flex flex-wrap gap-2">
...
function EquipmentExecutions({ plan, requests, canExecute, onExecute, onDocument }: { plan: PmocPlan; requests: PmocExecutionRequest[]; canExecute: boolean; onExecute: (equipment: EquipmentSummary) => void; onDocument: (item: PmocExecutionRequest) => void }) {
  ...
  return <div className="overflow-x-auto rounded-xl border border-[var(--color-border)]"><table className="w-full text-sm"><thead className="bg-[var(--color-muted)] text-left text-xs uppercase text-[var(--color-muted-foreground)]"><tr><th className="p-3">Equipamento</th><th className="p-3">Local</th><th className="p-3">Última execução</th><th className="p-3">Próxima prevista</th><th className="p-3">Status</th><th className="p-3">OS / Documento</th><th className="p-3 text-right">Ações</th></tr></thead><tbody>{equipments.map((equipment) => {
    const progress = plan.overview?.equipmentExecutions.find((item) => item.equipmentId === equipment.id);
    const equipmentRequests = requests.filter((request) => request.equipmentId === equipment.id);
    const latest = equipmentRequests
      .filter((request) => request.operation)
      .sort((left, right) => right.equipmentExecutionNumber - left.equipmentExecutionNumber)[0];
    const activeRequest = equipmentRequests
      .filter((request) => request.status === "PENDING" || request.status === "GENERATING_OS")
    ...
    const finished = progress ? !progress.hasOpenExecutions : false;
    return <tr key={equipment.id} className={`border-t border-[var(--color-border)] ${progress?.coverageEnded && progress.hasOpenExecutions ? "bg-amber-500/5" : ""}`}><td className="p-3"><Link href={`/equipamentos/${equipment.id}`} className="font-semibold text-[var(--color-primary)]">{equipment.name}</Link><span className="block text-xs text-[var(--color-muted-foreground)]">{[equipment.manufacturer, equipment.model, equipment.capacity].filter(Boolean).join(" · ") || equipment.tag || "Sem especificação"}</span>{progress && <span className="mt-1 block text-xs">{progress.completedExecutions}/{progress.expectedExecutions} concluídas</span>}</td><td className="p-3">{equipment.sector ?? equipment.address?.name ?? "Não informado"}</td><td className="p-3">{progress?.lastExecutionNumber ? <><strong>{String(progress.lastExecutionNumber).padStart(3, "0")}</strong><span className="block text-xs text-[var(--color-muted-foreground)]">{dateTime(progress.lastExecutionDate)}</span></> : "Ainda não executado"}</td><td className="p-3">{progress?.nextExecutionDate ? <><strong>{date(progress.nextExecutionDate)}</strong><span className="block text-xs text-[var(--color-muted-foreground)]">Execução {String(progress.nextExecutionNumber).padStart(3, "0")}</span></> : finished ? "Ciclos concluídos" : "A definir"}</td><td className="p-3">{progress?.coverageEnded && progress.hasOpenExecutions ? <StatusChip tone="danger">Cobertura encerrada · {progress.remainingExecutions} pendente(s)</StatusChip> : <StatusChip tone={equipmentProgressTone(progress?.executionStatus)}>{equipmentProgressLabel(progress?.executionStatus)}</StatusChip>}</td><td className="p-3">{latest?.operation ? <button className="text-left text-[var(--color-primary)]" onClick={() => onDocument(latest)}><span className="block">OS-{String(latest.operation?.number).padStart(6, "0")}</span><span className="text-xs">{document?.number ?? "Preparar documento"} · {latest.operation?._count?.photos ?? 0} evidência(s)</span></button> : "—"}</td><td className="p-3"><div className="flex justify-end gap-2">{latest?.operation && <button title="Preview, gerar e baixar PMOC" className={iconBtn} onClick={() => onDocument(latest)}><FileText className="h-4 w-4" /></button>}{canExecute && !finished && activeRequest?.status !== "GENERATING_OS" && <button title={`Executar PMOC para ${equipment.name}`} className={iconBtn} onClick={() => onExecute(equipment)}><Play className="h-4 w-4" /></button>}</div></td></tr>;
  })}</tbody></table></div>;
}
