...
export default function ClientesPage() {
  ...
  const columns = useMemo<Column<Customer>[]>(
    () => [
      ...
      { key: "doc", header: "Documento", className: "w-[180px]", cell: (c) => <span className="font-mono text-xs">{c.cnpj ?? c.cpf ?? "—"}</span> },
      { key: "phone", header: "Contato", className: "w-[170px]", cell: (c) => <span className="text-sm truncate">{c.phone ?? c.email ?? "—"}</span> },
      {
        key: "counts",
        header: "Vínculos",
        className: "w-[140px]",
      ...
      },
      {
        ...
        header: "Status",
        className: "w-[120px]",
        cell: (c) => <StatusPill status={c.isActive ? "success" : "offline"} label={c.isActive ? "Ativo" : "Inativo"} />,
      },
    ],
    [],
  );
    ...
    <div className="space-y-6 max-w-[1400px]">
      ...
      {list.loading && !list.data ? (
        ...
        <EmptyState
          ...
          description={debounced ? "Ajuste a busca e tente novamente." : "Cadastre o primeiro cliente para começar."}
        />
      ) : list.data ? (
