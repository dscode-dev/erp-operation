...
import { operationsApi, operationApi, useQuery, type DemoDocument, type DemoDocumentStatus, type DocumentsData } from "@erp/api";
...
export default function DocumentosPage() {
  ...
  // Real documents generated by Operations (OS drafts e demais documentos).
  ...
  // Map real OperationDocuments to the shared document-row shape and merge with
  // the demo central. Same kind/status unions, so filters/preview work as-is.
  const realDocs = useMemo<DemoDocument[]>(() => {
  ...
  const all = useMemo(() => [...realDocs, ...(docs.data?.items ?? [])], [realDocs, docs.data]);
  ...
  return (
    <div className="space-y-6 max-w-[1500px]">
      ...
      {(docs.loading || ops.loading) && all.length === 0 ? (
        <SkeletonList rows={6} />
      ) : docs.error && !docs.data && realDocs.length === 0 ? (
        <ErrorState error={docs.error} onRetry={docs.refetch} />
      ) : docs.data?.disabled && realDocs.length === 0 ? (
        <ComingSoonState title="Documentos em breve" description="Ative o Demo Dataset ou registre operações para visualizar a central de documentos." />
      ) : rows.length === 0 ? (
