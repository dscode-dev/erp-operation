...
export default function OperatorEquipamentosPage() {
  ...
  const list = useQuery(
    (signal) => equipmentsApi.listEquipments({ limit: 30, search: debounced || undefined, signal }),
    [debounced],
  );
  ...
  return (
    <div className="px-4 pt-4 pb-24 space-y-4">
      ...
      <div className="sticky top-12 z-10 -mx-4 px-4 py-2 bg-[var(--color-background)]/95 backdrop-blur">
        <SearchInput value={search} onChange={setSearch} placeholder="Nome, tag, série, modelo…" className="w-full" />
      </div>
      ...
      {list.loading && !list.data ? (
        ...
        <EmptyState icon={Wrench} title="Nenhum equipamento" description={debounced ? "Ajuste a busca." : "Sem equipamentos."} />
      ) : (
