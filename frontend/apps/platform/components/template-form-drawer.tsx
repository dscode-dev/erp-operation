...
import { organizationApi, ApiClientError, type DocumentTemplate, type DocumentTemplateType } from "@erp/api";
...
export function TemplateFormDrawer({
  ...
  async function save() {
    ...
    try {
      if (isEdit && template) {
        await organizationApi.updateTemplate(template.id, form);
      } else {
        await organizationApi.createTemplate({ type, ...form });
      }
      onSaved();
      onClose();
    } catch (err) {
...
function blank(): FormState {
  return { name: "", headerContent: "", footerContent: "", observations: "", isDefault: false, isActive: true };
}
