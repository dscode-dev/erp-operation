...
try {
  ...
  const operation = await api('/operations', { method: 'POST', headers, body: JSON.stringify({ customerId: customer.id, addressId: customer.addresses[0]?.id, equipmentId: equipments[0].id, operatorId: operator.id, type: 'PREVENTIVA', status: 'COMPLETED', startedAt: now.toISOString(), completedAt: now.toISOString(), maintenanceType: 'MONTHLY', inspectedEquipments: equipments.map((item, index) => ({ equipmentId: item.id, sector: `Setor ${index + 1}` })), maintenanceChecklist: [{ pmocUnit: 'EVAPORATOR', maintenanceType: 'MONTHLY', description: 'Limpar filtros e verificar condições operacionais', executed: true, result: 'YES', observations: 'Executado sem anormalidades' }, { pmocUnit: 'EVAPORATOR', maintenanceType: 'MONTHLY', description: 'Verificar drenagem', executed: false, result: 'NOT_APPLICABLE', observations: 'Item não aplicável ao conjunto' }, { pmocUnit: 'CONDENSER', maintenanceType: 'MONTHLY', description: 'Medir pressão e temperatura da condensadora', executed: true, result: 'YES', observations: 'Dentro dos parâmetros' }], observations: 'Execução PMOC concluída em campo.', photos: Array.from({ length: 4 }, (_, index) => ({ dataUrl: `data:image/png;base64,${png}`, caption: `Evidência PMOC ${index + 1}` })) }) });
  ...
  for (const id of ['pmoc-identification', 'pmoc-operational-data', 'pmoc-inspected-equipments', 'pmoc-legal-reference', 'pmoc-checklist', 'signature']) if (!preview.sections.some((section) => section.id === id)) throw new Error(`Missing ${id}`);
